...
const StickyRoll = () => {
  ...
  const buttonData = [
    {
      text: "STRATEGY",
      description: "Craft a comprehensive plan to achieve your business goals efficiently and effectively. Develop strategies tailored to your unique needs and challenges.",
      image: "algochurn.webp",
    },
    {
      text: "DESIGN",
      description: "Create visually stunning designs that captivate your audience and enhance user experience. From UI/UX design to branding, we bring your ideas to life.",
      image: "idk.png",
    },
    {
      text: "DEVELOPMENT",
      description: "Transform your concepts into robust, scalable applications using cutting-edge technologies. Our development team ensures high-quality solutions that meet your requirements.",
      image: "tailwindmasterkit.webp",
    },
    {
      text: "OPTIMIZATION",
      description: "Fine-tune your systems and processes to maximize efficiency and performance. Identify areas for improvement and implement optimization strategies for sustainable growth.",
  ...
  return (
    // main code
    <div className="flex justify-center items-center h-screen px-32 py-10">
      <div className="flex flex-col items-start justify-center space-y-8 p-4">
        ...
        >
          ...
          >
            ...
            >
              <Image
                src={`/${buttonData.find((button) => button.text === selected)?.image}`}
                alt={selected}
                width={600}
                height={600}
            ...
            <motion.p
              className="text-slate-50/[0.5] mt-8 text-start w-3/4"
              initial={{ opacity: 0, y: 50 }}
              animate={{ opacity: 1, y: 0 }}
              transition={{ duration: 0.2}}
            >
              {buttonData.find((button) => button.text === selected)?.description}
            </motion.p>
            <motion.button
              ...
              transition={{ duration: 0.2}}
            >
              <span className="spani text-white font-semibold text-xl">Work with us</span>
            </motion.button>
          </motion.div>
        </motion.div>
      </div>
      <div className="flex flex-col items-end justify-center space-y-10 p-4">
        {buttonData.map((button, index) => (
          <motion.button
            key={index}
            className={`text-7xl bg-clip-text font-black space-y-8  focus:outline-none ${
